"""
Example in CliffWalking, using the FiniteSarsaModel.
"""
import sys
sys.path.append("..")
...
for i in range(1, eps+1):
    ...
    # Decaying epsilon, reach optimal policy, only if it's an on-policy algorithm.
    m.epsilon = START_EPS*(eps-i)/eps

if SAVE_FIG:
    print("Final expected returns : {}".format(m.score(env, m.pi, n_samples=10)))
    utils.render_cliffwalking(m.Q, "cliffwalking_sarsa.png")
